...
Shell.prototype.generateFunctions = function(dryRun){
  _.each(this.resources, function(r){
    if(r instanceof Shell)  r.generateFunctions();
      ...
      if(r.steps.length == 1 && r.steps[0].checks.length == 0){
        if(dryRun){
          // no checks == never up2date
          w("    echo \"not up2date "+r.label+"\"");
        }else{
          w("  "  + this.wrap(r.steps[0].code));
          w('  if [[ "$?" -eq "0" ]]; then');
          w("    echo \"up2date ["+this.username+"] "+r.label+"\"");

          _.each(r.notif, function(n){
        ...
        w("  "+_.map(r.steps, function(s){ return this.wrap(s.checks.join(" && ")); }, this).join(" && "));
        w('  if [[ "$?" -eq "0" ]]; then');
        w("    echo 'up2date ["+this.username+"] "+r.label+"'");
        w("  else ");
        ...
        if(dryRun){
          ...
          for(var i=0; i < r.steps.length; i++){
            w("    "+this.wrap(r.steps[i].checks.join(" && ")) + " || " + this.wrap(r.steps[i].code));
            w("    "+_.map(r.steps.slice(0, i+1), function(s){ return this.wrap(s.checks.join(" && ")); }, this).join(" && "));
            w("    if [[ \"$?\" -eq \"0\" ]]; then");
            w("      echo \"updating ["+this.username+"] ("+(i+1)+"/"+r.steps.length+") "+r.label+"\"");
            w("    else");
            w("      echo \"not up2date ["+this.username+"] ("+(i+1)+"/"+r.steps.length+") "+r.label+"\"");
            w("      exit 1");
            w("    fi");
          }
          w("    echo \"up2date ["+this.username+"] "+r.label+"\"");
