...
var register = function ($){
  ...
  $.resource("start", function(name, opts, step){
    ...
    step("service "+name+" start && sleep "+opts.sleep)
      .eq("service "+name+" status 2>&1 | grep -E 'Running|is running|start/running' | wc -l", 1);
  });

  $.resource("stop", function(name, opts, step){
    opts.sleep = opts.sleep || 1;
    step("service "+name+" stop && sleep "+opts.sleep).eq("service "+name+" status 2>&1 | grep 'not running' | wc -l", 1);
  });
